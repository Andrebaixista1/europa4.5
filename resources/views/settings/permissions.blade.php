        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl">
                <div class="p-8 lg:p-10 text-gray-900">
                    ...
                    <div class="mb-8 rounded-xl border border-gray-200 bg-gray-50 px-5 py-4">
                    ...
                    <form method="post" action="{{ route('settings.permissions.update') }}" class="space-y-10">
                        ...
                        <div class="space-y-6">
                            @foreach ($hierarchies as $index => $hierarchy)
                                ...
                                <section x-data="{ open: {{ $index === 0 ? 'true' : 'false' }} }" class="rounded-2xl border border-gray-200 bg-white shadow-sm">
                                    <button
                                        type="button"
                                    ...
                                    >
                                        <div>
                                            <h3 class="text-xl font-semibold text-gray-900">{{ $hierarchy['label'] }}</h3>
                                        ...
                                        <div class="inline-flex h-9 w-9 items-center justify-center rounded-full border border-gray-300 text-gray-600">
                                        ...
                                        <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
                                            @foreach ($permissionsByGroup as $groupLabel => $permissions)
                                                <div class="rounded-xl border border-gray-200 bg-gray-50/60 p-4">
                                                    <h4 class="mb-3 text-xs font-semibold uppercase tracking-wide text-gray-500">{{ $groupLabel }}</h4>

                                                    <div class="space-y-2">
                                                        @foreach ($permissions as $permission)
                                                            <label class="flex items-center gap-3 rounded-lg px-2 py-2 text-sm text-gray-800 transition hover:bg-white">
                                                                <input
                                                                    type="checkbox"
